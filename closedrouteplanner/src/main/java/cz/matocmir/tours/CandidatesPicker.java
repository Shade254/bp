...
public class CandidatesPicker {
	...
	public Candidate selectCandidate(){
		...
		for (int j = 0; j<candidates.size(); j++) {
			double d = TourUtils.computeGreatCircleDistance(selected.correspNode.getNode(), tmp.next().correspNode.getNode());
	...
	public void setProbabilities() {
		...
		double lonScale = TourUtils.computeGreatCircleDistance(center.getLatitude(), center.getLongitude(), center.getLatitude(), center.getLongitude()+1);
		double latScale = TourUtils.computeGreatCircleDistance(center.getLatitude(), center.getLongitude(), center.getLatitude()+1, center.getLongitude()+1);
		...
		for (Candidate c: candidates) {
			TourNode cNode = c.correspNode.getNode();
			MaxSum cMaxSum = tileMaxSum.get(new IntPair((int) (cNode.getLatitude()/latTile), (int) (cNode.getLongitude()/lonTile)));

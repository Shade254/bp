...
public class ClosedTourMain {
	...
	private static void solve(TourGraph graph){
		...
		double strictness;
		System.out.print("Enter strictness: \n>");
		strictness = scan.nextDouble();
		...
		for(int i = 0;i<cands.size();i++) {
			...
			foundPath = backPathFinder.getPathBack(randomCand, startingNode, maxLength, minLength, strictness, factor);

			if (foundPath != null) {
				IOUtils.visualizeEdges(foundPath, "cycle_" + i + ".geojson");
				foundCycles++;
			} else {

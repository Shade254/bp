...
public class ClosedBackPathLabelFactory implements LabelFactory<BackPathLabel> {
	...
	/***
	 * method conducting the backward search for closed paths
	 * @param current currently expanded node
	...
	/***
	 * Counting edge cost by combining the edge pleaseantness and roundness against forward path
	 * @param e edge to count cost for
	 * @param length length from the turning point to current edge
	...
	private double countWeight(TourEdge e, double length) {
		...
		for (TourEdge e1 : forwardPath) {
			...
			double penalty = e.roundnessPenalty(e1, Math.min(distance, maxLength - distance), strictness);
